...
def save_results(
    ...
    if asset_class_dictionary:
        print(f"DEBUG #16 {results_sw['identifier']}")
        print(f"DEBUG #16 {type(results_sw['identifier'])}")
...
def run_single_window(
    ...
    print(f"path = {features_file_path}")
    ...
    # ------------------------------------------------------------------
    # Inference‑only path: skip any hyper‑parameter search or training
    # ------------------------------------------------------------------
    if skip_hp_search and best_hp_path is not None:
        # 1) load stored hyper‑parameters
        ...
        # no validation loss available in pure inference mode
        val_loss = float("nan")
    else:
        # full search / training path (kept for compatibility)
    ...
    save_results(
        ...
        asset_class_dictionary,
        {
            ...
            # "performance_fw": performance_fw,  # removed because fixed window is skipped
            "val_loss": val_loss,
        },
    )
